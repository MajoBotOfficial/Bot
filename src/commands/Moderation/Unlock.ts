...
export default class UnlockCommand extends MajoCommand {
	...
	override async messageRun(message: Message, args: Args) {
		const chan = await args.pick('guildTextChannel');
		const reason = await args.pick('string').catch(() => {});

		return this.ModClient.unlock(message, chan, reason as string);
	}
}
